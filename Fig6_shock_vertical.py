...
SLICE_EPSILON = 20.0 
OUTPUT_DIR = "Figures"
CACHE_DIR = "Cache_Initial_Matrix_With_MatID"  # 为了避免旧缓存冲突，改个名或者你可以手动删掉旧文件夹
...
def load_initial_tracers_slice(vtp_directory, step=0, slice_epsilon=1.0):
    ...
    for f in files:
        ...
        mask = np.abs(pts[:, 1]) < slice_epsilon
        if np.any(mask):
            # 筛选数据
            ...
            # [id, matid, x, z]
            # 注意：matid 可能是 float，这里也先存进去
            combined = np.column_stack((ids_filtered, mats_filtered, pts_filtered[:, 0], pts_filtered[:, 2]))
            pts_list.append(combined)

    if not pts_list: return None
    
    # 创建 DataFrame
    df = pd.DataFrame(np.vstack(pts_list), columns=['id', 'matid', 'x_init', 'z_init'])
...
def get_data_with_cache(folder_name, base_path, epsilon):
    cache_path = os.path.join(CACHE_DIR, f"{folder_name}_slice_mat.pkl")
    ...
    if df_initial is not None:
        # 只存需要的列
        data_to_save['background'] = df_initial[['matid', 'x_init', 'z_init']].copy()
        
    if not df_merged.empty:
        # Ejecta 也是用来画位置和颜色的，不需要 matid 也可以，但如果想根据材质筛选ejecta可以加上
        data_to_save['ejecta'] = df_merged[['x_init', 'z_init', 'pressure']].copy()
...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(3, 6, figsize=(18.4, 8), sharex=True, sharey=True)
    ...
    print("开始处理 3x6 矩阵数据 (Highlight matid=2)...")
    ...
    for i in range(3):
        for j in range(6):
            ...
            if df_bg is not None and not df_bg.empty:
                x_bg = df_bg['x_init'].values / R
                z_bg = df_bg['z_init'].values / R
                mat_bg = df_bg['matid'].values
                
                if i == 0:
                    # 第一行: 纯玄武岩，统一浅色
                    ax.scatter(x_bg, z_bg, c='gray', s=0.5, alpha=0.3, rasterized=True, zorder=1)
                else:
                    # 第二行 & 第三行: 区分材质
                    # 先画所有点作为浅色底 (或者只画非-2的点)
                    # 策略: 把 2 和 非2 分开画
                    ...
                    # 画岩石 (浅灰)
                    if np.any(mask_rock):
                        ax.scatter(x_bg[mask_rock], z_bg[mask_rock], 
                                   c='gray', s=0.5, alpha=0.3, rasterized=True, zorder=1)
                    
                    # 画冰 (深色 - 例如深灰色 dimgray 或 灰色 gray)
                    if np.any(mask_ice):
                        ax.scatter(x_bg[mask_ice], z_bg[mask_ice], 
                                   c='lightgray', # 这里可以改颜色，例如 'darkgray', 'dimgray'
            ...
            if df_ej is not None and not df_ej.empty:
                x_ej = df_ej['x_init'].values / R
                z_ej = df_ej['z_init'].values / R
                p_ej = df_ej['pressure'].values
                
                ax.scatter(x_ej, z_ej, c=p_ej, norm=norm, cmap=cmap, s=1.0, rasterized=True, zorder=2)
            
            ax.set_aspect('equal')
            ax.set_xlim(-1.8, 1.2) 
            ax.set_ylim(-1.0, 1.0)
            
            # if i == 0: ax.set_title(f"{ANGLES[j]}°", fontsize=14, pad=10)
            if i == 0: ax.set_title(f"Angle={ANGLES[j]}°", fontsize=15)
            if j == 0: 
                ax.set_ylabel(r"$Z_{init}/R_t$", fontsize=15)
                ax.tick_params(axis='y', labelsize=15)
            if i == 2: 
                ax.set_xlabel(r"$X_{init}/R_t$", fontsize=15)
    ...
    print("处理完成，正在保存图像...")
    # 定义输出文件名 (不带后缀)
    output_filename = os.path.join(OUTPUT_DIR, "Fig6_Peak_Shock_Pressure")
